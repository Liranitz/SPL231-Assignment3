...
if __name__ == '__main__':
    ...
    for con in active_cons:
        if con.is_connected():
            ...
            time.sleep(0.5)
    if conn4 is not None and conn2.is_connected():
        print('conn 4 should not be connected and we disconnected it', flush=True)
        conn4.disconnect()
    time.sleep(0.5)
for con in active_cons:
    assert not con.is_connected(), f"{con} should be disconnected"
if conn4 is not None:
    assert not conn4.is_connected(), f"conn4 should be disconnected"